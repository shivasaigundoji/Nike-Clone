        ...
        <div className="flex flex-1 justify-between lg:gap-10 gap:20 flex-wrap">
          {
            footerLinks.map(section=>(
              <div className="text-white">
                <h4 className="text-montserrat text-2xl leading-normal font-medium mb-6">{section.title}</h4>
                <ul>
                  {
                    section.links.map((link)=>(
                      <li key={link.name} className="mt-3 text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray cursor-pointer"><a href={link.link}>{link.name}</a></li>
                    ))
                  }
                </ul>
              </div>
